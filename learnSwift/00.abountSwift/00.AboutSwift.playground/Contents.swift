...
/**
    ...
    * 스위프트가 함수형 프로그래밍 패러다임을 차용한 의의
    ...
    스위프트는 객체지향과 함수형 패러다임을 모두 차용하고 있다. 애플의 프레임워크 대부분은 객체지향 프로그래밍 패러다임을 기반으로 설계된 수많은 클래스로 구성되어있다. 따라서 애플의 프레임워크에서 사용될 언어가 객체지향 프로그래밍 패러다임을 수용하지 않는다면 당연히 문제가 된다.
 
    하지만 애플은 스위프트에서 함수형 프로그래밍 패러다임을 특히나 강조한다. 이는 애플의 프레임워크를 벗어나 다른 영역(eg. 서버용 프레임워크 등)에서 스위프트를 사용했을 때 순수하게 함수형 프로그래밍 패러다임만으로 프로그램을 작성할 수 있기 때문이다.
 
        -- 순수하게 함수형 프로그래밍 패러다임으로 작성하면..
        1. 여러 가지 연산 처리 작업이 동시에 일어나는 프로그래밍을 만들기 쉽다.
        2. 멀티 코어 혹은 여러 개 연산 프로세서를 사용하는 시스템에서 효율적인 프로그램을 만들기 쉽다.
        3. 상태변화에 따른 부작용에서 자유로와지므로 순수하게 기능 구현에 초점을 맞춰서 설계할 수 있다.
        ---
    
    때에 따라서 객체지향, 함수형 패러다임을 섞어서 사용하면 기능에 맞게 최적의 성능을 발휘할 수 있다.
 
    > 3. 프로토콜 지향
    스위프트에서는 구조체와 열거형에 기존 클래스에서 구현할 수 있었던 캡슐화, 추상화, 접근 제어 등의 기능을 모두 구현할 수 있다. 더불어 프로토콜에 익스텐션을 활용할 수 있기 때문에 프로토콜 지향 프로그래밍이 가능해졌다. 이는 참조 타입의 참조 문제에서 조금 더 자유로울 수 있고, 다중 상속이 불가능한 한계를 뛰어넘을 수 있으며, 더 나은 추상화 메커니즘을 구현할 수 있다는 이야기이다. 실제로 스위프트의 프로토콜 지향 프로그래밍 기법은 프로토콜 익스텐션 기능을 통해 완성되었으며 강력해졌다.
 */
